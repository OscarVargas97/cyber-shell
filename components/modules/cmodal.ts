...
if grep -qE '^Hidden=true|^X-GNOME-Autostart-enabled=false' "$DST"; then
  ...
  printf '\\nX-GNOME-Autostart-enabled=false\\n' >> "$DST"
    ...
    const fetchSysInfo = () => {
      ...
      sh("nix-store -q --requisites /run/current-system 2>/dev/null | wc -l").then((o) => { st.sysInfo.pkgs = o.trim() || "—"; ctrl.requestDraw() })
...
export const CModalWindows = () => [register(VolCtrl()), register(BrtCtrl()), register(WifiCtrl()), register(BtCtrl()), register(PwrCtrl()), register(BatCtrl()), register(AiUsageCtrl()), register(KeysCtrl()), register(ThemesCtrl())]
